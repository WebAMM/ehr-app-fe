...
export default function DoctorDetailsPage({
  ...
  const { data: doctorData, isLoading: doctorLoading } =
    useGetDoctorDetailsQuery(
      { userId: userId },
      { skip: isClinic && stateData },
    );
  const [
    clinicDoctorProfile,
    { data: doctorDetailsData, isLoading: doctorDetailsLoading },
  ] = useClinicDoctorProfileMutation();
  useEffect(() => {
    if (isClinic && stateData) {
      clinicDoctorProfile({
        clinicId: stateData?.clinicId,
        body: { clinicDoctorId: stateData?.doctorID },
      });
    }
  }, [isClinic, stateData]);
  ...
  const doctorDetails =
    isClinic && stateData ? doctorDetailsData?.data : doctorData?.data[0];
  const handleAppointmentClick = () => {
    navigate(
      isClinic ? "/clinic-doctor-appointment" : "/doctor-booking-appointment",
      {
        state: {
          doctorDetails: doctorDetails,
          clinicId: stateData?.clinicId,
          doctorID: stateData?.doctorID,
        },
      },
    );
  };
  ...
  return (
    <main className="min-h-screen bg-pageBackground">
      ...
      />
      {doctorLoading || doctorDetailsLoading ? (
        ...
        <div className="px-4 sm:px-6 lg:px-8 py-8 grid grid-cols-1 lg:grid-cols-3 gap-8">
                  ...
                  {isClinic ? (
                    ...
                    <div className="flex items-center gap-2 mt-2 flex-wrap">
                      ...
                      <div className="text-muted-foreground text-sm">
                        ({doctorDetails?.reviews?.totalReviews || "N/A"}{" "}
                        reviews)
                      </div>
                      <div className="text-muted-foreground text-sm">
          ...
          <div className="lg:sticky lg:top-24 h-fit">
            <Card className="p-6 border border-secondary/30">
              ...
              ) : doctorDetails?.type === "Video Consultation" ? (
                <div
...
function StatItem({ label, value }) {
  return (
    <div className="text-center bg-secondary/10 rounded-lg p-3 flex-1">
