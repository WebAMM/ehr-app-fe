...
const Map = ({ doctorDetails }) => {
  ...
  useEffect(() => {
    if (doctorDetails?.address || doctorDetails?.location) {
      setMapLoading(true);
      getCoordinates(doctorDetails?.address || doctorDetails?.location)
        .then((data) => {
          if (data) {
            setCoords({
  ...
  }, [doctorDetails?.address, doctorDetails?.location]);
  return (
    <Card className="p-6">
