...
import {
  registerSchema,
  userUpdateSchema,
} from "@/pages/Authentication/AuthValidation";
...
const UserProfileForm = ({ isEditMode = false, userData }) => {
  ...
  const [updateUserProfile, { isLoading: isUpdating }] =
    useUpdateUserProfileMutation();
  const isLoading = isEditMode ? isUpdating : isRegistering;
  ...
  useEffect(() => {
    if (isEditMode && userData?.attachDoc) {
      setProfileImage(userData.attachDoc);
    }
  }, [isEditMode, userData?.attachDoc]);
  ...
  const getInitialValues = () => {
    if (isEditMode && userData) {
      return {
        ...
        terms: false,
        bloodGroup:
          userData.bloodGroup && userData.bloodGroup !== "undefined"
            ? userData.bloodGroup
            : "",
        attachDoc: userData.attachDoc || "",
        weight:
          userData.weight && userData.weight !== "undefined"
            ? userData.weight
            : "",
      };
    }
    ...
    if (isEditMode) {
      try {
        ...
        editableFields.forEach((field) => {
          if (values[field] !== initialVals[field]) {
            formData.append(field, values[field]);
          }
        });
        if (selectedFile) {
          formData.append("attachDoc", selectedFile);
        }
        ...
        toastError(error?.data?.message || "Update failed. Please try again.");
      }
    } else {
      try {
  ...
  return (
    <Formik
      ...
      enableReinitialize
      validationSchema={isEditMode ? userUpdateSchema : registerSchema}
